...
bool Map::hasWall(MapBlock& currentBlock, MapBlock& nextBlock)
{
    ...
    {
        return units[currentX][currentY].hasWall(Direct::UP);
    }
    else if (nextX == currentX && nextY > currentY)
    {
        return units[currentX][currentY].hasWall(Direct::DOWN);
    }
    else if (nextX < currentX && nextY == currentY)
    {
        return units[currentX][currentY].hasWall(Direct::LEFT);
    }
    else if (nextX > currentX && nextY == currentY)
    {
        return units[currentX][currentY].hasWall(Direct::RIGHT);
    }
    return true;
}
...
void Map::removeWall(MapBlock& currentBlock, MapBlock& nextBlock)
{
    ...
    if (nextX == currentX && nextY < currentY)
    {
        units[currentX][currentY].removeWall(Direct::UP);
        units[nextX][nextY].removeWall(Direct::DOWN);
    }
    else if (nextX == currentX && nextY > currentY)
    {
        units[currentX][currentY].removeWall(Direct::DOWN);
        units[nextX][nextY].removeWall(Direct::UP);
    }
    else if (nextX < currentX && nextY == currentY)
    {
        units[currentX][currentY].removeWall(Direct::LEFT);
        units[nextX][nextY].removeWall(Direct::RIGHT);
    }
    else if (nextX > currentX && nextY == currentY)
    {
        units[currentX][currentY].removeWall(Direct::RIGHT);
        units[nextX][nextY].removeWall(Direct::LEFT);
    }
}
